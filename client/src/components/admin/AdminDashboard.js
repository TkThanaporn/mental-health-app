...
import {
  ...
  FaUserShield,
  FaFileExcel,
  FaPrint, // ✅ นำเข้า FaUserShield เพิ่ม
} from "react-icons/fa";
...
const DASHBOARD_FALLBACK_YEARS = [
  ...new Set([new Date().getFullYear(), 2025]),
].sort((a, b) => b - a);
...
const AdminDashboard = () => {
  ...
  const fetchStats = async (token) => {
    try {
      ...
      const res = await axios.get(
        `http://localhost:5000/api/admin/summary?year=${selectedYear}`,
        {
          headers: { "x-auth-token": token },
        },
      );
  ...
  const handleExportExcel = async () => {
    ...
    try {
      setExporting(true);
      const res = await axios.get(
        `http://localhost:5000/api/admin/export/excel?year=${selectedYear}`,
        {
          headers: { "x-auth-token": token },
          responseType: "blob",
        },
      ...
      link.setAttribute(
        "download",
        `pcshs-heartcare-report-${toBuddhistYear(selectedYear)}.xls`,
      );
  ...
  const handleOpenPrintableReport = async () => {
    ...
    try {
      setExporting(true);
      const res = await axios.get(
        `http://localhost:5000/api/admin/export/report?year=${selectedYear}`,
        {
          headers: { "x-auth-token": token },
          responseType: "text",
        },
    ...
    ...new Set(
      [
        ...(stats.availableYears?.length
          ...
          : DASHBOARD_FALLBACK_YEARS),
        selectedYear,
      ].map(Number),
    ),
  ].sort((a, b) => b - a);
  const roleChartData = stats.roleSummary?.length
    ? stats.roleSummary
    : [
  ...
  const dormitoryChartData = stats.dormitoryUsage?.length
    ? stats.dormitoryUsage
    : [{ dormitory: "ไม่มีข้อมูล", count: 0 }];
  const gradeChartData = stats.gradeUsage?.length
    ? stats.gradeUsage
    : ["ม.1", "ม.2", "ม.3", "ม.4", "ม.5", "ม.6"].map((grade) => ({
        grade,
        count: 0,
      }));
  const roleTotal = roleChartData.reduce(
    ...
    <div className="d-flex flex-column h-100">
      ...
      <div className="mt-auto p-4 border-top border-secondary border-opacity-25">
        <Button
          ...
          className="text-white-50 text-decoration-none p-0 d-flex align-items-center hover-danger"
        >
  ...
  const renderContent = () => {
    switch (activeTab) {
      case "dashboard":
        return (
          <div className="fade-in">
            {/* Hero Section */}
            <div
              className="hero-welcome p-4 p-lg-5 mb-5 d-flex align-items-center justify-content-between shadow-lg"
              style={{
                borderRadius: "24px",
                background: "linear-gradient(135deg, #003566 0%, #001d36 100%)",
              }}
            >
              ...
              <div className="position-relative z-1">
                <Badge
                  bg="danger"
                  text="white"
                  className="mb-2 px-3 rounded-pill fw-bold"
                >
                  Super Admin
                </Badge>
                <h1 className="fw-title display-5 fw-bold mb-2 text-white">
                  สวัสดีคุณ {adminProfile.fullname}
                </h1>
                <p
                  className="text-white-50 mb-4 fw-light lead"
                  style={{ maxWidth: "600px" }}
                >
                  ควบคุมและจัดการระบบดูแลช่วยเหลือนักเรียน <br />{" "}
                  โรงเรียนวิทยาศาสตร์จุฬาภรณราชวิทยาลัย
                </p>
                <Button
                  className="btn-pcshs shadow-lg px-4 py-2"
            ...
            <div className="px-2">
              <div className="dashboard-section-heading mb-4">
                <div>
                  <h4 className="fw-bold mb-1" style={{ color: "#003566" }}>
                    สถานะระบบปัจจุบัน
                  </h4>
                  <p className="text-muted mb-0">
                    เลือกปีเพื่อดูแนวโน้มคำขอและกลุ่มนักเรียนที่ใช้บริการ
                  </p>
                </div>
                ...
                <Dropdown align="end" className="export-dropdown">
                  ...
                  <Dropdown.Menu className="export-menu shadow-sm border-0">
                    ...
                    <Dropdown.Item onClick={handleOpenPrintableReport}>
                      <FaPrint className="me-2 text-orange" /> พิมพ์ / Save as
                      PDF
                    </Dropdown.Item>
                  </Dropdown.Menu>
                </Dropdown>
              ...
              <Row className="g-4 mb-4">
                {[
                  ...
                  {
                    title: `คำขอปี ${toBuddhistYear(selectedYear)}`,
                    count: stats.yearly_appointments,
                    unit: "รายการ",
                  ...
                  {
                    title: "นักจิตวิทยา",
                    count: stats.pending_psychologists,
                    unit: "คน",
                    icon: <FaUserMd />,
                    type: "stat-warning",
                  },
                  ...
                  <Card className="dashboard-chart-card">
                    <div className="chart-card-header">
                      <div>
                        ...
                        <p>
                          จำนวนนักเรียนที่มีคำขอในปี{" "}
                          {toBuddhistYear(selectedYear)}
                        </p>
                      </div>
                    </div>
                    {loadingStats ? (
                  ...
                  <Card className="dashboard-chart-card">
                    <div className="chart-card-header">
                      <div>
                        ...
                        <p>
                          จำนวนนักเรียน ม.1 ถึง ม.6 ในปี{" "}
                          {toBuddhistYear(selectedYear)}
                        </p>
                      </div>
                    </div>
                    {loadingStats ? (
                      ...
                      <ResponsiveContainer width="100%" height={330}>
                        <BarChart
                          data={gradeChartData}
                          margin={{ top: 8, right: 18, bottom: 4, left: -12 }}
                        >
                          <CartesianGrid
                            strokeDasharray="3 3"
                            stroke={CHART_COLORS.grid}
      ...
      case "profile":
        return <AdminProfile />; // ✅ 3. ผูกคำสั่งเมื่อกดแท็บ 'profile' ให้ดึงหน้า AdminProfile มาแสดง
      default:
        return null;
    }
  };
  ...
  return (
    <div className="dashboard-wrapper">
      <div className="pcshs-sidebar d-none d-lg-block shadow">
        <SidebarContent />
      </div>
      ...
      <Offcanvas
        ...
        style={{ width: "280px", background: "#003566" }}
      >
